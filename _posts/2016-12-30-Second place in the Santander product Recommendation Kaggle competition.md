...
header-img: "img/Santander office 4 shaded.jpg"
comments: true
---
...
The interactive base model prediction application made me think of various ways to combine the base model predictions. I tried several weighted transformations but could not find a transformation that consistently works better than a simple weighted average of the base model predictions. Different weights were used for different products. Cco for example showed particularly different behavior in June and December 15 compared to other months and it seemed like a mixed distribution of typical cco patterns and end of (tax) year specific patterns. The table below shows the relative lag weights for all products. These are all normalised so that they sum to 1 for each product.
{% include image.html url="/img/product weights.png" description="Relative base model weights by product" %}

Some product positive flanks such as nomina and nom_pens mostly rely on information from the previous lag but product positive flanks like recibo get more confident when more lag information is available. Let's say for example that recibo was dropped in Oct 15 and picked up in Nov 15. The June 15 model would not be able to use this information since only 5 test lag months are used to evaluate the model on the test set (Jan 15 - May 15). In these cases I adjusted the probability to the probabilities of the models that use more data in some of my submissions. The next plot compares the weighted prediction for May 2016 using the weights from the table above with the adjusted predictions that only incorporate a subset of the lags. The adjusted (purple) predictions are way closer to the predictions using the out-of-bag May 2016 model compared to the weighted approach that uses all base lags and can thus be seen as preferable for the studied user.
